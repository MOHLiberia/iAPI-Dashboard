...
<template>
  <nav class="dark:border-slate-800 border-r lg:flex flex-col hidden shrink-0" :class="props.open ? 'w-[250px]' : 'w-fit'">
      ...
      <div class="rounded flex flex-col border dark:border-slate-800" v-for="list in menuList" :class="list.openSubmenu ? '' : ''">
        <div class="flex flex-1 dark:hover:bg-slate-800">
          <NuxtLink :to="list.to" class="flex-1 flex gap-2 items-center p-2">
            ...
            <span v-if="props.open">{{list.name}}</span>
          </NuxtLink>
    ...
    <div class="p-2">
      <NuxtLink class="w-full rounded border dark:border-slate-800 dark:hover:bg-slate-800 hover:bg-slate-100 p-2 group flex items-center gap-2" to="/dashboard/account" v-if="open">
        <div class="h-10 w-10 rounded-full border-slate-800 bg-green-300"/>
        <div class="flex flex-col text-sm" v-if="open">
