...
<template>
	<Card class="w-full md:w-[500px]">
		<template #title>
			<div class="w-full text-left text-xl">Reset Password</div>
		</template>
		<template #subtitle>
			<div class="w-full text-left mb-2">Type in and verify your new password below to change it.</div>
		</template>
		<template #content>
			<Form :resolver class="flex flex-col gap-3 w-full" @submit="resetPassword">
				<div class="w-full space-y-3">
					...
					<FormField v-slot="$field" :validateOnValueUpdate="false" class="space-y-1" initialValue=""
							   name="confirmPassword" validate-on-blur>
						<FloatLabel variant="on">
