        ...
        <Form :resolver>
          <div class="w-full flex flex-col gap-2">
            ...
            <div class="grid md:grid-cols-2 gap-3">
              <FormField v-slot="$field" name="username" initialValue="" class="space-y-1">
            ...
            <Divider align="left" type="solid" class="col-span-4">
              <b>Profile Details</b>
            </Divider>
            <div class="space-y-3">
              ...
              <div class="grid md:grid-cols-2 gap-3">
                <FormField v-slot="$field" name="countyId" initialValue="" class="space-y-1">
                  <FloatLabel variant="on" class="md:col-span-1">
                    <Select id="countyId" :options="counties?.data?.sort((a, b) => a.name.localeCompare(b.name))" filter optionLabel="name" optionValue="id" :loading="pendingCounties" fluid showClear />
                    <label for="countyId">County of Assignment</label>
                  </FloatLabel>
                  <Message v-if="$field.invalid" severity="error" size="small" variant="simple">{{ $field.error.message }}</Message>
                </FormField>
                <FormField v-slot="$field" name="organizationId" initialValue="" class="space-y-1">
                  <FloatLabel variant="on" class="md:col-span-1">
                    <Select id="organizationId" v-model="selectedOrganization" :options="organizations?.data?.sort((a, b) => a.name.localeCompare(b.name))" filter optionLabel="name" optionValue="id" :loading="pendingOrganizations" fluid showClear />
                    <label for="organizationId">Organization</label>
                  </FloatLabel>
                  <Message v-if="$field.invalid" severity="error" size="small" variant="simple">{{ $field.error.message }}</Message>
                </FormField>
                <FormField v-slot="$field" name="departmentId" initialValue="" class="space-y-1">
                  <FloatLabel variant="on" class="md:col-span-1">
                    <Select id="departmentId" :options="filteredDepartments" filter optionLabel="name" optionValue="id" fluid showClear />
