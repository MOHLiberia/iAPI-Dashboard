...
<template>
  <div>
    <DataTable :value="requests?.data" class="text-nowrap" :loading="pending" size="small" showGridlines>
      <Column field="firstName" header="First Name"/>
      <Column field="middleName" header="Middle Name"/>
      <Column field="lastName" header="Last Name"/>
      <Column field="email" header="Email"/>
      <Column field="organization" header="Organization"/>
      <Column field="department" header="Department" class="!w-fit"/>
      <Column field="county.name" header="County Name" class="!w-fit"/>
      ...
      <template #footer>There are <span class="font-semibold">{{ requests ? requests?.data.length : 0 }}</span> application request(s). </template>
    </DataTable>
  </div>
  {{requests}}
</template>
